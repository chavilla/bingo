...
const CanvasComponent = () => {
  ...
  useEffect(() => {
    ...
    const books_storaged_pending = JSON.parse(localStorage.getItem("books_bible_storaged")) || null;
  ...
  const getRandomArbitrary = (min, max) => {
    ...
    localStorage.setItem("books_bible_storaged", JSON.stringify(books_bible_pending));
    ...
    <div className="canvas_component_box">
      <div className="btn_shake_box">
        <button
          ...
          className="btn_shake"
          disabled={finished}
          onClick={() => getRandomArbitrary(0, books_bible_pending.length)}
        >
          Elegir libro
        </button>
      </div>
      ...
      <div className="books_list_box">
        <h2 className="text-center">Libros generados { books_storaged_state.length}/66 </h2>
